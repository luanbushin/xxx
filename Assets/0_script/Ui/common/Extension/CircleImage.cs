...
namespace Game.UI.Extension
{
    ...
    public class CircleImage : BaseImage
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            Vector4 uv =
                overrideSprite != null ?
                DataUtility.GetOuterUV(overrideSprite) : Vector4.zero;
            ...
            if (fill) //圆形
            {
                ...
                uiVertex.uv0 =
                    new Vector2(
                        curVertice.x * uvScaleX + uvCenterX,
        ...
        private bool Contains(
            Vector2 p, List<Vector3> outterVertices, List<Vector3> innerVertices)
        {
            var crossNumber = 0;
        ...
        private void RayCrossing(
            Vector2 p, List<Vector3> vertices, ref int crossNumber)
        {
            for (int i = 0, count = vertices.Count; i < count; i++)
            {
                ...
                if (((v1.y <= p.y) && (v2.y > p.y))
                    || ((v1.y > p.y) && (v2.y <= p.y)))
                {
                    // 只考虑点击点右侧方向，点击点水平线与线段相交，
                    // 且交点x > 点击点x，则crossNumber+1
